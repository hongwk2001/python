...
def mono(image, sepia=False):
    ...
    # Change ths to return True when the function is implemented
    ...
    for row in range(height):
        for col in range(width):
            ...
            pixel.green = int(brightness * ( 0.6 if sepia else 1))
            pixel.blue = int(brightness * (0.4 if sepia else 1))
    ...
    if vertical :
        ...
        for i in range(height):
            ...
            for j in range(width):
                image[new_row][j] = nimg[i][j]

    else:
        for row in range(height):
            new_row =[]
            for col in range(width):
                new_row.insert(0,image[row][col])
            
        #  print (row, image[row], new_row  )
            image.pop(row)
            image.insert(row, new_row)
...
def blur(image,radius=5):
    ...
    def blur1(i,j,r):
        fr_i = 0 if i-r < 0 else i-r
        to_i = cols if i +r > cols else i +r 
        fr_j = 0 if j-r < 0 else j-r
        to_j = rows if j +r > rows else j + r

    def get_average(i,j,r):
        fr_i = 0 if i-r < 0 else i-r
        to_i = cols if i +r > cols else i +r 
        fr_j = 0 if j-r < 0 else j-r
        to_j = rows if j +r > rows else j + r
