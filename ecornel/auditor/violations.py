...
def bad_visibility(visibility,minimum):
    ...
    """
    if visibility ==  'unavailable': return True
    
    avg_visibility = visibility['prevailing']
    units= visibility['units']

    if units == 'FT':
        avg_visibility = visibility['minimum'] / 5280

    return avg_visibility < minimum
...
def bad_winds(winds,maxwind,maxcross):
    ...
    #print(winds)
    if winds  == 'unavailable': return True
    if winds == 'calm': return False

    speed= winds['speed']
    crosswind=winds.get('crosswind',None)
    gusts    =winds.get('gusts',None)

    if winds['units'] == 'MPS' :
        speed=  speed * 1.94384
        if crosswind is not None : crosswind= crosswind * 1.94384
        if gusts is not None : gusts=  gusts * 1.94384

    if speed > maxwind: return True

    if gusts is not None and gusts > maxwind: return True

    if crosswind is not None and crosswind > maxcross: return True
...
def bad_ceiling(ceiling,minimum):
    ...
    Precondition: minimum is a float or int
    """
   # print(ceiling, minimum)
    if ceiling  == 'unavailable': return True
    if ceiling == 'clear': return False

    # pop if ceiling has  a few, scattered
    newceiling=[]
    for row in ceiling:
        if row['type'] not in ('a few' ,'scattered'):
            newceiling.append(row)

    if len(newceiling) ==0 : return False

    height=newceiling[0]['height']
    type1=newceiling[0]['type']
...
def get_weather_report(takeoff,weather):
    ...
    #iso_time = takeoff.isoformat()

    from datetime import datetime, timedelta

    trim_takeoff = takeoff.replace(minute=0,second=0,microsecond=0)
    iso_time = trim_takeoff.isoformat()
    #print(iso_time)
    try:
        return weather [iso_time]
    except KeyError:
        trim_minus_one = trim_takeoff - timedelta(hours=1)
        #print('pick', trim_minus_one.isoformat())
        return weather [trim_minus_one.isoformat()]
...
def get_weather_violation(weather,minimums):
    ...
    #'Unknown' It returns if no weather reading is available (e.g. weather is None).  
    if weather is None : return 'Unknown'

    bad_vis=False
    if  bad_visibility(weather['visibility'],minimums[1]): bad_vis = True

    bad_wind =False
    if bad_winds(weather['wind'],minimums[2],minimums[3]): bad_wind =True

    bad_ceil = False 
    if  bad_ceiling(weather['sky'],minimums[0]): bad_ceil = True

    #print ( bad_vis , bad_wind , bad_ceil )

    #only one bad covered first 
    if bad_vis and not bad_wind and not bad_ceil : return 'Visibility'
    if bad_wind and not bad_vis and not bad_ceil : return 'Winds'
    if bad_ceil and not bad_wind and not bad_vis : return 'Ceiling'
    # multiple problems should be remain
    if bad_vis or bad_wind or bad_ceil : return 'Weather'
...
def list_weather_violations(directory):
    ...
    for lesson in lessons :
        ...
        #Get the pilot credentials  students.csv
        student = utils.get_for_id(lesson[0],students)
        #print (takeoff,student)
        cert=pilots.get_certification(takeoff,student)
    
        #Get the pilot minimums   minimums.csv
        vfr = True if lesson[5] == 'VFR' else False
        daytime = utils.daytime(takeoff, daycycle)
        area = lesson[6]
        instructed = True if lesson[2] > '' else False
        # Get the pilot minimums
        #Returns the most advantageous minimums
        adv_minums =pilots.get_minimums(cert, area, instructed, vfr, daytime, minimums)

        # Get the weather conditions
        # Returns the most recent weather report
        weather_rpt = get_weather_report(takeoff, weather)

        # Check for a violation and add to result if so
        weather_violation = get_weather_violation(weather_rpt,adv_minums)
        if weather_violation > '' :
            lesson.append ( weather_violation)
