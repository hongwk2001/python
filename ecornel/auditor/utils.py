...
def daytime(time,daycycle):
    ...
    ntime = str_to_time(time.isoformat(),"America/New_York")
    #print(ntime, type(ntime))
    try:
        str_sunrise_time = daycycle[str(ntime.year)][ntime.strftime('%m-%d')]['sunrise']
        str_sunset_time  = daycycle[str(ntime.year)][ntime.strftime('%m-%d')]['sunset']
    except KeyError:
        return None

    srtime = str_to_time(ntime.strftime('%Y%m%dT') + str_sunrise_time, "America/New_York")
    sstime = str_to_time(ntime.strftime('%Y%m%dT') + str_sunset_time, "America/New_York")

    #print(ntime, srtime, sstime)
    return ntime >srtime and ntime < sstime
